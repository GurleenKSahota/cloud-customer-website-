...
resource "aws_api_gateway_resource" "restock" {
  rest_api_id = aws_api_gateway_rest_api.pos_api.id
  parent_id   = aws_api_gateway_resource.inventory.id
  path_part   = "restock"
}

resource "aws_api_gateway_method" "restock_post" {
  rest_api_id      = aws_api_gateway_rest_api.pos_api.id
  resource_id      = aws_api_gateway_resource.restock.id
  http_method      = "POST"
  authorization    = "NONE"
  api_key_required = true
}

resource "aws_api_gateway_integration" "restock_post" {
  rest_api_id             = aws_api_gateway_rest_api.pos_api.id
  resource_id             = aws_api_gateway_resource.restock.id
  http_method             = aws_api_gateway_method.restock_post.http_method
  type                    = "HTTP_PROXY"
  integration_http_method = "POST"
  uri                     = "http://${aws_instance.pos_server.public_ip}:3001/inventory/restock"
}
...
resource "aws_api_gateway_integration" "products_get" {
  ...
  type                    = "HTTP_PROXY"
  integration_http_method = "GET"
  uri                     = "http://${aws_instance.pos_server.public_ip}:3001/inventory/products"
}
...
resource "aws_api_gateway_deployment" "pos_deployment" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_method.restock_post,
      ...
      aws_api_gateway_integration.restock_post,
